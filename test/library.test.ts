...
test("Libraries", async () => {
  await appTest(UnitTest.DO_NOT_PRINT_INITIALIZED_LOG, async () => {
    await testInWorkspace(async (app, org, ws) => {
      ...
      await expect(app.administration.libraries.addOrgToLibrary(libraryId, org.id)).resolves.not.toThrow()

      // await app.userProfile.fetch()
      // expect(app.organizations.get(org.id).isAllowedInLibraries).toBe(true)

      let libraries = await app.administration.libraries.getLibraries()
      ...
      await expect(app.administration.libraries.deleteOrgFromLibrary(libraryId, org.id)).resolves.not.toThrow()

      libraries = await app.administration.libraries.getLibraries()
      ...
      await expect(app.administration.libraries.deleteLibrary(libraryId)).resolves.not.toThrow()
