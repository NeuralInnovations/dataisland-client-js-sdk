...
test("Organization prompts", async () => {
  await appTest(UnitTest.DO_NOT_PRINT_INITIALIZED_LOG, async () => {
    await testInOrganization(async (app, org) => {
      ...
      // get default prompts
      const prompts = await org.prompts.getPrompts(chatSource)
      expect(prompts.length).toEqual(0)

      // add prompt
      await org.prompts.updatePrompt(chatSource,"test", "test")

      // check get prompts
      const prompts2 = await org.prompts.getPrompts(chatSource)
      expect(prompts2.length).toEqual(1)

      // check update prompts
      await org.prompts.updatePrompts(chatSource, prompts)

      // length should be 1
      const prompts3 = await org.prompts.getPrompts(chatSource)
      expect(prompts3.length).toEqual(1)

      // check delete prompt
      await org.prompts.deletePrompt(chatSource,"test")
      const prompts4 = await org.prompts.getPrompts(chatSource)
      expect(prompts4.length).toEqual(0)

      // check update prompts in bulk
      await org.prompts.updatePrompts(chatSource, [{
        key: "test",
        value: "test"
      }, {
        key: "test2",
      ...
      const prompts5 = await org.prompts.getPrompts(chatSource)
      ...
      const prompts6 = await org.prompts.getPrompts(chatSource)
...
test("Organization default prompts", async () => {
  await appTest(UnitTest.DO_NOT_PRINT_INITIALIZED_LOG, async () => {
    await testInOrganization(async (app, org) => {
      const quizSource = "quiz"
      const prompts = await org.prompts.getDefaultPrompts(quizSource)
      expect(prompts.length).toBeGreaterThan(0)

      await org.prompts.updatePrompts(quizSource, prompts)
      const currentPrompts = await org.prompts.getPrompts(quizSource)
      expect(currentPrompts.length).toEqual(prompts.length)

      await org.prompts.deletePrompts(quizSource, prompts.map(p => p.key))
      const currentPrompts2 = await org.prompts.getPrompts(quizSource)
